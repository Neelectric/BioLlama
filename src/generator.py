...
class ExLlamaGenerator:
    ...
    class Settings:
        ...
        temperature = 0.2      #standard: 0.95
        top_k = 40                              # consider the most probable top_k samples, 0 to disable top_k sampling
        top_p = 0.95        #standard: 0.65                            # consider tokens up to a cumulative probabiltiy of top_p, 0.0 to disable top_p sampling
        ...
        token_repetition_penalty_max = 2 #standard: 1.15     # Repetition penalty for most recent tokens
    ...
    def gen_begin_reuse(self, in_tokens, mask = None):
        ...
        while reuse < self.sequence.shape[-1] and reuse < in_tokens.shape[-1] and self.sequence[0, reuse] == in_tokens[0, reuse]:
            reuse += 1
        #Neel: adding this line because im suspicious of this "reuse" code...
        reuse = 0
        if reuse < 2:
            self.gen_begin(in_tokens, mask = mask)
            return 0
    ...
    def gen_single_token(self, constraints = None, mask = None):
        ...
        if self.sequence is not None:
            ...
            naive_prob = torch.softmax(logits,dim=-1).reshape([-1])
            ...
            token, _ = self.batched_sample(logits,
                                           ...
                                           self.settings.typical)
            # print(f"naive prob shape {naive_prob.shape}")
            naive_prob = naive_prob[token]
    ...
    def beam_search(self):
        ...
        if self.settings.beams == 1 and self.settings.beam_length == 1: return self.gen_single_token()[0]
        assert self.in_beam_search

        # Kludge: The first token returned with an empty context is generated without beam search
        if self.sequence is None: return self.gen_single_token()[0]
